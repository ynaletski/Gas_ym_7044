...
const unsigned char name_icp[6][2] = {{49,50},{49,55},{50,50},{50,52},{52,52},{56,48}};  //nm was:{49,50} - 12 ,{49,55} - 17 ,{50,50} - 22 ,{50,52} - 24 ,{54,48} - 7060 ,{56,48} - 80
const unsigned char mvs_rd[5]={3,0,68,0,7};/* команда короткое чтение */                                                                            //now:{52,52} - 7044
    ...
    if (Port[1].status == 4) /* обработка ответа от MVS */
    {
      ...
      if (typ_pool < 5)
      {
        ...
        CalcCRC16(Port[1].buf,i,cr);
	      if (Port[1].buf[i]==cr[1] && Port[1].buf[i+1]==cr[0]) ReadFromMVS(Prt.nmb);
        /*else printf("errMVS%d %d %d %d",Port[1].buf[0],Port[1].buf[1],Port[1].buf[2],Port[1].buf[3]);*/
      } 
...
void SendToMVS (unsigned char number)
{
  ...
  if (Sensor[number].wait == 0)
  {
    ...
    { 
      bufs[0]=255;
      bufs[1]=255;
      bufs[2]=Device.adr_mvs[number];
      switch (Sensor[number].evt)
      {

	      case 0:
          for (i=0;i< 5;i++) bufs[3+i]=mvs_rd[i];/*короткое чтение*/
	        count=8;
          typ_pool=1;
        break;

	      case 1:
          for (i=0;i< 5;i++) bufs[3+i]=mvs_rdl[i];/*длинное чтение*/
	        count=8;
          typ_pool=2;
        break;

	      case 2:
          for (i=0;i< 6;i++) bufs[3+i]=mvs_wr[i];/*запись статуса*/
        ...
        break;
	      ...
	      case 3:
          for (i=0;i< 6;i++) bufs[3+i]=mvs_cbr[i];
          bufs[9]=mvs_cmd[0];
	        bufs[10]=mvs_cmd[1];
    ...
    switch (evt)
    {
      ...
      case 3:if (status==5)
	     {  /*установка дискретных выходов 7060* @xxF !!!!!!!!!!  7044 @xxFF */
	       bufs[0]=Key_adr;count=5;evt=2; //nm was (count=4; for 7060) (count=5; for 7044)
...
void ReadFromMVS (unsigned char nmb_dev)
{
  ...
  switch (typ_pool)
  {
    case 1:
      for (i=0;i< 4;i++)
	    {
	      ConvToFloatVerify(&Sensor[nmb_dev].data[i],Port[1].buf[5+i*4],Port[1].buf[6+i*4],Port[1].buf[7+i*4],Port[1].buf[8+i*4]);
	    } 
    ...
    case 2:
      for (i=0;i< 10;i++) Sensor[nmb_dev].setup[i+3]=Port[1].buf[i+3];
	    ...
	    for (i=0;i< 21;i++)
	    {
	      ConvToFloatVerify(&Sensor[nmb_dev].data[i],Port[1].buf[19+i*4],Port[1].buf[20+i*4],Port[1].buf[21+i*4],Port[1].buf[22+i*4]);
	    } 
      for (i=0;i<3;i++) Sensor[nmb_dev].avg[i]=Sensor[nmb_dev].data[i+1];
